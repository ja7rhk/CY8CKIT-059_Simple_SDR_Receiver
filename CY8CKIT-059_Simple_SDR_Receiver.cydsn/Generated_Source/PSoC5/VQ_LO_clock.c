...
/*******************************************************************************
* Function Name: VQ_LO_clock_Start
********************************************************************************
*
* Summary:
...
void VQ_LO_clock_Start(void) 
{
    ...
    VQ_LO_clock_CLKEN |= VQ_LO_clock_CLKEN_MASK;
	VQ_LO_clock_CLKSTBY |= VQ_LO_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: VQ_LO_clock_Stop
********************************************************************************
*
* Summary:
...
void VQ_LO_clock_Stop(void) 
{
    ...
    VQ_LO_clock_CLKEN &= (uint8)(~VQ_LO_clock_CLKEN_MASK);
	VQ_LO_clock_CLKSTBY &= (uint8)(~VQ_LO_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: VQ_LO_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((VQ_LO_clock_CLKEN & VQ_LO_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = VQ_LO_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = VQ_LO_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* VQ_LO_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(VQ_LO_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(VQ_LO_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: VQ_LO_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: VQ_LO_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = VQ_LO_clock_GetSourceRegister();
    uint16 oldDivider = VQ_LO_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = VQ_LO_clock_CLKEN & VQ_LO_clock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((VQ_LO_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: VQ_LO_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(VQ_LO_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: VQ_LO_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    VQ_LO_clock_MOD_SRC |= modeBitMask & (uint8)VQ_LO_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: VQ_LO_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    VQ_LO_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(VQ_LO_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: VQ_LO_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return VQ_LO_clock_MOD_SRC & (uint8)(VQ_LO_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: VQ_LO_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = VQ_LO_clock_GetDividerRegister();
    uint8 oldSrc = VQ_LO_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        VQ_LO_clock_MOD_SRC |= CYCLK_SSS;
        VQ_LO_clock_MOD_SRC =
            (VQ_LO_clock_MOD_SRC & (uint8)(~VQ_LO_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: VQ_LO_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return VQ_LO_clock_MOD_SRC & VQ_LO_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: VQ_LO_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    VQ_LO_clock_PHASE = clkPhase & VQ_LO_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: VQ_LO_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return VQ_LO_clock_PHASE & VQ_LO_clock_PHASE_MASK;
}
