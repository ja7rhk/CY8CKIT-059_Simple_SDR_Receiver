...
/*******************************************************************************
* Function Name: CyVI_LO_CLK_Start
********************************************************************************
*
* Summary:
...
void CyVI_LO_CLK_Start(void) 
{
    ...
    CyVI_LO_CLK_CLKEN |= CyVI_LO_CLK_CLKEN_MASK;
	CyVI_LO_CLK_CLKSTBY |= CyVI_LO_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CyVI_LO_CLK_Stop
********************************************************************************
*
* Summary:
...
void CyVI_LO_CLK_Stop(void) 
{
    ...
    CyVI_LO_CLK_CLKEN &= (uint8)(~CyVI_LO_CLK_CLKEN_MASK);
	CyVI_LO_CLK_CLKSTBY &= (uint8)(~CyVI_LO_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CyVI_LO_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CyVI_LO_CLK_CLKEN & CyVI_LO_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CyVI_LO_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CyVI_LO_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CyVI_LO_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CyVI_LO_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CyVI_LO_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CyVI_LO_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CyVI_LO_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CyVI_LO_CLK_GetSourceRegister();
    uint16 oldDivider = CyVI_LO_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CyVI_LO_CLK_CLKEN & CyVI_LO_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CyVI_LO_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CyVI_LO_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CyVI_LO_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: CyVI_LO_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CyVI_LO_CLK_MOD_SRC |= modeBitMask & (uint8)CyVI_LO_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: CyVI_LO_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CyVI_LO_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CyVI_LO_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: CyVI_LO_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CyVI_LO_CLK_MOD_SRC & (uint8)(CyVI_LO_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: CyVI_LO_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CyVI_LO_CLK_GetDividerRegister();
    uint8 oldSrc = CyVI_LO_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CyVI_LO_CLK_MOD_SRC |= CYCLK_SSS;
        CyVI_LO_CLK_MOD_SRC =
            (CyVI_LO_CLK_MOD_SRC & (uint8)(~CyVI_LO_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CyVI_LO_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CyVI_LO_CLK_MOD_SRC & CyVI_LO_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CyVI_LO_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CyVI_LO_CLK_PHASE = clkPhase & CyVI_LO_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CyVI_LO_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CyVI_LO_CLK_PHASE & CyVI_LO_CLK_PHASE_MASK;
}
